...
--
-- Both surviving views were keyed to the retired MFT financial persona and
-- would silently report zero now that /opt/mft is gone from the sensor.
--
-- v_cred_attempts: cred_tier tested usernames against a hardcoded list of
-- planted MFT lures (svc_mft, mftadmin, sftpadmin, mft_fedline and others).
...
-- '%/opt/mft%', a path that no longer exists, and nothing in app/ reads it.
