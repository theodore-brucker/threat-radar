    ...
    h("div", { class: "grid g2" }, [
      panel("Pipeline", null, [
        ...
        P("Transport: pull model over Tailscale against a forced-command key, so the sensor "
          + "holds no credentials for the Pi and the Pi cannot be reached from the sensor."),
      ...
      panel("Analysis", null, [
        ...
        P("Campaign clustering: credential pairs are tagged by rule, so a hundred "
          + "variations of one theme read as one campaign instead of a hundred rows."),
        ...
        P("Spike attribution: detection uses a rolling median with median absolute "
